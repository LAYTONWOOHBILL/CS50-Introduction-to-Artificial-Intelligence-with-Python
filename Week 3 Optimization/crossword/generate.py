...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        """
        # print(self.crossword.height)
        # print(self.crossword.width)
        # print(self.crossword.structure)
        # print(self.crossword.words)
        # print(self.crossword.overlaps)

        # Recall that node consistency is achieved when, for every variable, each value in its domain is consistent
        # with the variable’s unary constraints. In the case of a crossword puzzle, this means making sure that every
        # value in a variable’s domain has the same number of letters as the variable’s length.
    ...
    def revise(self, x, y):
        ...
        # Recall that x is arc consistent with y when every value in the domain of x has a
        # possible value in the domain of y that does not cause a conflict.
        # (A conflict in the context of the crossword puzzle is a square for which two variables disagree
        # on what character value it should take on.)
                ...
                if x_value != y_value:
                    ...
                    if self.crossword.overlaps[(x, y)]:
                        ...
                        if x_value[x_index] == y_value[y_index]:
                            consistent = True
                            break
            # To make x arc consistent with y, you’ll want to remove any
            # value from the domain of x that does not have a corresponding possible value in the domain of y.
            # The domain of y should be left unmodified.
            if not consistent:
                self.domains[x].discard(x_value)
                is_revision = True


        # The function should return True if a revision was made to the domain of x;
        # it should return False if no revision was made.
        return is_revision
    ...
    def ac3(self, arcs=None):
        ...
        # you remove all of the remaining values from a domain, return False
        # (this means it’s impossible to solve the problem, since there are no more possible values for the variable).
        #
    ...
    def consistent(self, assignment):
        ...
        # An assignment is consistent if it satisfies all of the constraints of the problem: that is to say, all values are distinct,
        # every value is the correct length, and there are no conflicts between neighboring variables.
        ...
        for variable, value in assignment.items():
            ...
            for neighbor in neighbors:
                ...
                if neighbor in assignment:
                    ...
                    overlap_index_x, overlap_index_y = self.crossword.overlaps[(variable, neighbor)]
                    if value[overlap_index_x] != neighbor_value[overlap_index_y]:
                        return False
        return True
    ...
    def order_domain_values(self, var, assignment):
        ...
        # Recall that the least-constraining values heuristic is computed
        # as the number of values ruled out for neighboring unassigned variables.
        # That is to say, if assigning var to a particular value results in eliminating
        # n possible choices for neighboring variables, you should order your results in ascending order of n.
        ...
        # Once your algorithm is working, you can then go back and ensure that the values are returned in the correct order.
        ...
        number_of_values_ruled_out = {val: 0 for val in self.domains[var]}

        for values in self.domains[var]:
            if not values in assignment:
                neighbors = self.crossword.neighbors(var)
                for neighbor in neighbors:
                    if values in self.domains[neighbor]:
                        number_of_values_ruled_out[values] += 1

        return sorted(number_of_values_ruled_out, key=lambda key: number_of_values_ruled_out[key])
    ...
    def select_unassigned_variable(self, assignment):
        ...
        # If there is a tie between variables,
        ...
        # (which should still generate correct crossword puzzles).
        # Once your algorithm is working, you can then go back
        # and ensure that you are returning a variable according to the heuristics.
        # You may find it helpful to sort a list according to a particular key:
        # Python contains some helpful functions for achieving this.
    ...
    def backtrack(self, assignment):
        ...
        # your function should return the complete assignment:
        ...
        # If you would like, you may find that your algorithm is more efficient if you interleave search with inference
        # (as by maintaining arc consistency every time you make a new assignment).
        # You are not required to do this, but you are permitted to, so long as your function still produces correct results.
        # (It is for this reason that the ac3 function allows an arcs argument, in case you’d like to start with a different queue of arcs.)
