    ...
    with open(filename) as csvfile:
        ...
        for row in reader:
            ...
            for i in range(len(row)):
                ...
                # Browser, Region, TrafficType, VisitorType, and Weekend should all be of type int
                ...
                if i in [10]:
                    evidence.append(int(Month[row[i]]) - 1)
                # VisitorType should be 1 for returning visitors and 0 for non-returning visitors.
                if i in [15]:
                    evidence.append(1 if row[-1] == "Returning_Visitor" else 0)
                # Weekend should be 1 if the user visited on a weekend and 0 otherwise.
                if i in [16]:
                    evidence.append(1 if row[-1] == "TRUE" else 0)
                # Each value of labels should either be the integer 1,
